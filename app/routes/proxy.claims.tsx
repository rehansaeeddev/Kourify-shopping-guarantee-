...
<body>
  ...
  <script>
    (function(){
      ...
      function submit(){var data=values();next.disabled=true;next.textContent=tr("state.submitting");fetch("/apps/kourify/claim",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(data)}).then(function(response){return response.json().catch(function(){return{}}).then(function(json){return{ok:response.ok,json:json}})}).then(function(result){if(!result.ok)throw new Error(result.json.error||tr("error.submitFailed"));form.innerHTML='<div class="success"><div class="success-mark">✓</div><h3>'+safe(tr("success.title"))+'</h3><p>'+tr("success.body").replace("{email}",safe(data.email))+'</p></div>';document.querySelector(".progress").hidden=true;document.querySelector(".legal").hidden=true}).catch(function(reason){next.disabled=false;next.textContent=tr("action.submit");showError(reason.message||tr("error.generic"))})}
